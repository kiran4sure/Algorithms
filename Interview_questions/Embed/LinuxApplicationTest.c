...
static void* Reader_thread(void* arg)
{
    while(!Done)
    {
        ...
        random_len = (random_len > Data_length)? Data_length : random_len;        
        ...
        while(random_len)
        {
            ...
            // un-comment for testing, limiting data to 4 full chucks
            // Reader_count++;
            // if(Reader_count % sizeof(DATA_BASE) == 0)
            //     printf("\n\n");
            // if(Reader_count == 4*sizeof(DATA_BASE))
            // {
            //     Done = TRUE;
            //     break;
            // }
...
static void* Writer_Thread(void* arg)
{
    while(!Done)
    {
        ...
        while(random_len > 0)
        {
            ...
            // while(((Data_end+1) % BUFF_LEN != Data_start) && random_len--) 
            while((Data_end+1)%BUFF_LEN != Data_start && random_len--) 
            {
